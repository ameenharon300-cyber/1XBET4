// ===================================================
// 🚀 AI GOAL PREDICTOR ULTIMATE - VERSION 10.9
// 👤 DEVELOPER: AMIN - @GEMZGOOLBOT
// 🔥 FEATURES: SMART AI + BETTING SYSTEM + FIREBASE + FULL ADMIN PANEL
// ===================================================

console.log('🤖 بدء تشغيل AI GOAL Predictor Ultimate v10.9...');
...
// 🧠 SMART GOAL PREDICTION ENGINE - UPDATED FOR SIMPLE GOAL/NO GOAL
class GoalPredictionAI {
    constructor() {
        this.algorithmVersion = "10.9";
    }
...
// 🎯 ADVANCED IMAGE VALIDATION WITH ENHANCED DETECTION
class ImageValidator {
    ...
    async validateImage(imageUrl) {
        try {
            console.log('🔍 جاري التحقق من الصورة باستخدام الذكاء الاصطناعي...');
                ...
                messages: [
                    {
                        ...
                        content: [
                            {
                                type: "text",
                                text: `تحقق بدقة شديدة من الصورة التالية:

مطلوب التحقق من أن الصورة تحتوي على واجهة لعبة GOAL الأصلية التي تحتوي على:
1. نص كبير واضح "GOAL!" أو كلمة "هدف" في الأعلى
2. لاعبين كرة قدم مشهورين (مثل نيمار، ميسي، رونالدو)
3. أزرار خضراء وحمراء واضحة مكتوب عليها "هدف" و "لا هدف"
4. واجهة لعبة كرة القدم مع شاشة اللعبة
5. زر "وضع الرهان"

إذا كانت الصورة تحتوي على كل هذه العناصر بوضوح، أجب بـ "مباراة".
إذا كانت الصورة لا تحتوي على أي من هذه العناصر (صورة شخص، خلفية، مباراة حقيقية، أي شيء آخر غير اللعبة)، أجب بـ "مرفوضة".
...
أجب فقط بكلمة واحدة: "مباراة" أو "مرفوضة".`
            ...
            console.log('نتيجة التحقق من OpenAI:', answer);
            ...
            // في حالة الخطأ، نستخدم التحقق البصري الأساسي
            return await this.basicImageValidation(imageUrl);
        }
    }

    async basicImageValidation(imageUrl) {
        try {
            // تحقق أساسي باستخدام وصف الصورة
            const response = await axios.post('https://api.openai.com/v1/chat/completions', {
                model: "gpt-4o-mini",
                messages: [
                    {
                        ...
                        content: [
                            {
                                type: "text",
                                text: `هل هذه صورة لعبة كرة قدم تحتوي على لاعبين وأزرار خضراء وحمراء؟ أجب بـ نعم أو لا فقط.`
                            },
                            {
                                type: "image_url",
                                image_url: imageUrl
            ...
            const answer = response.data.choices[0].message.content.toLowerCase().trim();
            return {
                valid: answer.includes('نعم'),
                message: answer.includes('نعم') ? 
                    '✅ تم التعرف على صورة لعبة GOAL - جاري التحليل...' :
                    '❌ هذه ليست صورة اللعبة، أرسل صورة من داخل لعبة GOAL فقط.',
                confidence: answer.includes('نعم') ? 0.8 : 0.7
            };
        } catch (error) {
            console.error('خطأ في التحقق الأساسي:', error);
            // كحل أخير، نقبل الصورة لتجنب حظر المستخدمين
            return {
                valid: true,
...
// 📢 CHANNEL BROADCAST SYSTEM - IMPROVED
...
const channelChecker = new ChannelSubscriptionChecker(bot, "@GEMZGOOL");
        ...
        if (callbackData.startsWith('win_') || callbackData.startsWith('lose_')) {
            ...
            if (isWin) {
                ...
                await channelManager.sendPredictionMessage(userData, 
                    { type: '⚽ GOAL - هدف', emoji: '⚽', probability: 85, confidence: 95, reasoning: 'توقع هدف ناجح - فوز' }, 
...
bot.on('text', async (ctx) => {
    try {
        ...
        console.error('خطأ في معالج النص:', error);
        await ctx.replyWithMarkdown('❌ حدث خطأ غير متوقع', getMainKeyboard());
    }
});

// 🖼️ IMAGE ANALYSIS HANDLER - IMPROVED WITH STRICT VALIDATION
bot.on('photo', async (ctx) => {
    try {
        ...
        // التحقق من صحة الصورة باستخدام الذكاء الاصطناعي - تحقق صارم
...
${userData.subscription_status !== 'active' ? 
    ...
    `✅ *اشتراك نشط - محاولات غير محدودة*`}
            ...
            console.error('خطأ في التحليل:', analysisError);
...
${userData.subscription_status !== 'active' ? 
    ...
    `✅ *اشتراك نشط - محاولات غير محدودة*`}
        ...
        // إرسال التحليل للقناة
        await channelManager.sendPredictionMessage(userData, prediction, ctx.session.currentBet);
...
5. انتظر التفعيل من الإدارة
        ...
        console.error('خطأ في الاشتراكات:', error);
...
async function handleSubscriptionSelection(ctx, userData, text) {
    ...
    try {
        ...
        console.error('خطأ في اختيار الاشتراك:', error);
...
async function handlePaymentScreenshot(ctx, userId) {
    try {
        ...
        await channelManager.sendPaymentMessage(paymentData);
...
async function handleAdminCommands(ctx, text) {
    ...
    try {
        ...
        switch (text) {
            ...
            case '💰 تعديل سعر أسبوعي':
                ctx.session.editingPrices = true;
                ctx.session.currentEditingType = 'week';
                await ctx.replyWithMarkdown(
                    `💰 *تعديل سعر أسبوعي*\n\n` +
            ...
            case '💰 تعديل سعر شهري':
                ctx.session.editingPrices = true;
                ctx.session.currentEditingType = 'month';
                await ctx.replyWithMarkdown(
                    `💰 *تعديل سعر شهري*\n\n` +
                    `💵 الرجاء إدخال السعر الجديد (بالدولار):\n\n` +
                    `📝 مثال: 30 أو 35.5`
            ...
            case '💰 تعديل سعر 3 أشهر':
                ctx.session.editingPrices = true;
                ctx.session.currentEditingType = 'three_months';
                await ctx.replyWithMarkdown(
                    `💰 *تعديل سعر 3 أشهر*\n\n` +
                    `💵 الرجاء إدخال السعر الجديد (بالدولار):\n\n` +
                    `📝 مثال: 80 أو 90`
            ...
            case '💰 تعديل سعر سنوي':
                ctx.session.editingPrices = true;
                ctx.session.currentEditingType = 'year';
                await ctx.replyWithMarkdown(
                    `💰 *تعديل سعر سنوي*\n\n` +
                    `💵 الرجاء إدخال السعر الجديد (بالدولار):\n\n` +
                    `📝 مثال: 250 أو 300`
            ...
            case '🔗 تعديل رابط شهري':
                ...
                await ctx.replyWithMarkdown(
                    ...
                    `🌐 الرجاء إدخال الرابط الجديد:\n\n` +
                    `📝 يمكنك إدخال أي رابط أو حتى نص وصورة\n` +
                    `📸 مثال: https://example.com/payment/monthly`
                );
                break;

            case '🔗 تعديل رابط 3 أشهر':
                ctx.session.editingLinks = true;
                ctx.session.currentEditingType = 'three_months';
                await ctx.replyWithMarkdown(
                    `🔗 *تعديل رابط 3 أشهر*\n\n` +
                    `🌐 الرجاء إدخال الرابط الجديد:\n\n` +
                    `📝 يمكنك إدخال أي رابط أو حتى نص وصورة\n` +
                    `📸 مثال: https://example.com/payment/3months`
                );
                break;

            case '🔗 تعديل رابط سنوي':
                ctx.session.editingLinks = true;
                ctx.session.currentEditingType = 'year';
                await ctx.replyWithMarkdown(
                    `🔗 *تعديل رابط سنوي*\n\n` +
                    `🌐 الرجاء إدخال الرابط الجديد:\n\n` +
                    `📝 يمكنك إدخال أي رابط أو حتى نص وصورة\n` +
                    `📸 مثال: https://example.com/payment/yearly`
                );
                break;
                
            default:
                await ctx.replyWithMarkdown('🔧 *تم معالجة الأمر بنجاح*', getAdminMainKeyboard());
                break;
        }
    } catch (error) {
        console.error('خطأ في أوامر الإدمن:', error);
        await ctx.replyWithMarkdown('✅ *تم تنفيذ الأمر بنجاح*', getAdminMainKeyboard());
    }
}
    ...
    try {
        ...
        for (const user of users) {
            try {
                ...
                failed++;
                console.log(`فشل إرسال للمستخدم ${user.user_id}:`, error.message);
            }
        }
        ...
        console.error('خطأ في الإشعار الجماعي:', error);
...
async function handleAdminToggleMaintenance(ctx) {
    try {
        ...
        console.error('خطأ في تبديل الصيانة:', error);
...
📝 *اختر نوع السعر الذي تريد تعديله:*
        ...
        console.error('خطأ في إعدادات الأسعار:', error);
...
📝 *اختر نوع الرابط الذي تريد تعديله:*
        ...
        console.error('خطأ في روابط الدفع:', error);
...
• سنوي: ${settings.payment_links?.year || 'غير محدد'}
        ...
        console.error('خطأ في الإعدادات العامة:', error);
...
async function handleAdminUsersList(ctx) {
    try {
        ...
        console.error('خطأ في قائمة المستخدمين:', error);
...
async function handleAdminActiveUsers(ctx) {
    try {
        ...
        console.error('خطأ في المشتركين النشطين:', error);
...
async function handleAdminFreeUsers(ctx) {
    try {
        ...
        console.error('خطأ في المستخدمين المجانين:', error);
...
async function handleAdminPendingPayments(ctx) {
    try {
        ...
        console.error('خطأ في الطلبات المعلقة:', error);
...
async function handleAdminAcceptedPayments(ctx) {
    try {
        ...
        console.error('خطأ في الطلبات المقبولة:', error);
...
async function handleAdminRejectedPayments(ctx) {
    try {
        ...
        console.error('خطأ في الطلبات المرفوضة:', error);
...
💰 الإجمالي: ${payments.length}
        ...
        console.error('خطأ في كل الطلبات:', error);
...
async function handlePriceInput(ctx, text) {
    try {
        ...
        if (isNaN(price)) {
            await ctx.replyWithMarkdown('❌ *السعر يجب أن يكون رقماً!*\n\n💵 يرجى إدخال سعر صحيح:');
            return;
        }
        ...
        console.error('خطأ في إدخال السعر:', error);
...
async function handleLinkInput(ctx, text) {
    try {
        // ✅ يقبل أي نص - رابط، نص عادي، أو حتى وصف
        const settings = await dbManager.getSettings();
...
async function handlePaymentAccept(ctx, paymentId) {
    try {
        ...
        try {
            ...
            console.log('لا يمكن تعديل الرسالة:', editError);
        }
...
// 🚀 START BOT
bot.launch().then(() => {
    console.log('🎉 نجاح! AI GOAL Predictor v10.9 يعمل الآن!');
