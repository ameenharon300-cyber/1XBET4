...
// 🔧 CONFIGURATION (READING FROM RENDER ENVIRONMENT VARIABLES)
const CONFIG = {
    // يجب وضع هذه المتغيرات في إعدادات Render
    BOT_TOKEN: process.env.BOT_TOKEN,
    ADMIN_ID: process.env.ADMIN_ID,
    CHANNEL_ID: process.env.CHANNEL_ID, // 👈 تم إضافة هذا
    
    // 🧠 AI APIS
    AI_APIS: {
        GEMINI: process.env.GEMINI_API_KEY,
        OPENAI: process.env.OPENAI_API_KEY // للحاجة المستقبلية
    },
...
// ⚠️ التحقق من المتغيرات الأساسية للتشغيل على Render
if (!CONFIG.BOT_TOKEN || !CONFIG.ADMIN_ID || !CONFIG.AI_APIS.GEMINI || !CONFIG.CHANNEL_ID) {
    console.error("CRITICAL ERROR: Please define BOT_TOKEN, ADMIN_ID, GEMINI_API_KEY, and CHANNEL_ID in Render environment variables.");
    // إذا كنت تستخدم Render، فإن process.exit(1) سيؤدي إلى فشل البناء.
    // لكنه ضروري لضمان عدم تشغيل البوت بدون مفاتيح.
    // عملية الإنهاء هنا تضمن عدم استمرار البوت في العمل بدون التوكنات اللازمة.
    // process.exit(1);
}
...
const ai = new GoogleGenAI(CONFIG.AI_APIS.GEMINI);

// 🌐 HEALTH CHECK SERVER FOR RENDER/REPLIT (Code remains the same)
const app = express();
...
try {
    ...
    // ... (Your existing Firebase serviceAccount configuration block)
    const serviceAccount = {
        "type": "service_account",
        "project_id": "bot-tlegram-9f4b5",
        "private_key_id": process.env.FIREBASE_PRIVATE_KEY_ID || "private_key_id",
        // يجب أن يتم تمرير مفتاحك الخاص هنا بشكل صحيح من متغيرات البيئة في Render
...
// 🗄️ LOCAL STORAGE FALLBACK (Code remains the same)
...
// 📊 FAKE STATISTICS SYSTEM (Code remains the same)
class FakeStatistics {
    // ... (Existing FakeStatistics class)
    constructor() {
...
// 🧠 SMART GOAL PREDICTION ENGINE (تم تعديل دالة analyzeImageWithAI)
class GoalPredictionAI {
    ...
    generateSmartPrediction(userId) {
        // دالة التوقع العشوائي البديلة (للاستخدام الداخلي أو في حالة الفشل)
    ...
    // دالة مساعدة لتحويل الـ Buffer إلى جزء يمكن للـ Gemini قراءته
    ...
    // 🎯 تنفيذ تحليل الصورة باستخدام Gemini Vision 
    async analyzeImageWithAI(imageUrl, mimeType = 'image/jpeg') {
        let imageBuffer;
        
        try {
            // 1. جلب الصورة من رابط تليجرام باستخدام axios
            ...
            // 2. إعداد الطلب لـ Gemini Vision
            const imagePart = this.fileToGenerativePart(imageBuffer, mimeType);
            
            // هذا الموجه (Prompt) يركز على التحقق من أن الصورة هي واجهة اللعبة المطلوبة
            const prompt = `
                حلل هذه الصورة وأجبني عن هذه الأسئلة:
                1. هل هي واجهة لعبة الرهان "1xBet Goal"؟
                2. هل تحتوي على عناصر اللعبة الواضحة (مثل شعار GOAL! أو أزرار الرهان)؟
                
                بناءً على تحليلك، أجب بكلمة واحدة فقط: 'MATCH' إذا كانت الصورة مطابقة للواجهة، أو 'NO_MATCH' إذا لم تكن كذلك.
                ثم، في سطر جديد، ضع توقعك التالي (Goal أو No Goal) مع سبب بسيط. 
                مثال:
                MATCH
                Goal. The strikers are in a strong position.
            `;
            ...
            const [matchResult, predictionText] = aiResponse.text.trim().split('\n');

            const isMatch = matchResult && matchResult.toUpperCase().includes('MATCH');
            const finalPrediction = predictionText || this.generateSmartPrediction('fallback').type;
            
            if (isMatch) {
                const isGoal = finalPrediction.toLowerCase().includes('goal');
                const probability = Math.floor(Math.random() * 20) + 75; // احتمالية عالية للثقة

                return {
                    type: isGoal ? '⚽ هدف مؤكد' : '🛡️ دفاع قوي',
                    probability: probability,
                    confidence: 100,
                    reasoning: finalPrediction,
                    timestamp: new Date().toISOString(),
                    algorithm: this.algorithmVersion
                };
            } else {
                // إذا لم تتطابق الصورة، نرجع رسالة فشل واضحة
                return {
                    type: '❌ عدم تطابق الصورة',
                    probability: 0,
                    confidence: 0,
                    reasoning: 'الصورة غير مطابقة لواجهة لعبة "1xBet Goal" المطلوبة للتحليل.',
                    timestamp: new Date().toISOString(),
                    algorithm: this.algorithmVersion
                };
            }
            ...
            // في حالة فشل الاتصال، نستخدم التوقع الداخلي
...
// 📤 IMGBB UPLOADER (Code remains the same)
class ImgBBUploader {
    // ... (Existing ImgBBUploader class)
    constructor(apiKey) {
        this.apiKey = apiKey;
    }
...
// 💾 DATABASE MANAGER (Code remains the same)
class DatabaseManager {
    // ... (All existing methods like getUser, saveUser, getAllUsers, getSettings, etc.)
    constructor() {
...
// 🎯 لوحة المفاتيح الثابتة (Code remains the same)
...
// 🛠️ UTILITY FUNCTIONS (Code remains the same)
...
// 🎯 BOT COMMANDS (Code remains the same)
bot.start(async (ctx) => {
    try {
...
// 📝 HANDLE TEXT MESSAGES (Code remains the same)
...
// 🖼️ IMAGE ANALYSIS HANDLER - تم تعديل هذه الدالة لإضافة إرسال للقناة
bot.on('photo', async (ctx) => {
    try {
        ...
        const mimeType = photo.mime_type || 'image/jpeg'; // استخدام mimeType لتحليل Gemini
        ...
        try {
            // استخدام دالة تحليل Gemini الحقيقية
            const prediction = await goalAI.analyzeImageWithAI(imageUrl, mimeType);
            
            await ctx.deleteMessage(processingMsg.message_id);

            // التحقق من أن الصورة مطابقة
            if (prediction.type.includes('عدم تطابق')) {
                await ctx.replyWithMarkdown(`❌ *فشل التحليل*\n\n💡 ${prediction.reasoning}`, getMainKeyboard());
                return;
            }
...
${userData.subscription_status !== 'active' ? 
    ...
    `✅ *اشتراك نشط - محاولات غير محدودة*`}
            ...
            // 📢 إرسال التوقع إلى القناة (تمت إضافة هذا الجزء)
            try {
                const channelMessage = `
                    📢 *توقع جديد من نظام AI GOAL Predictor* 🤖
                    
                    🎯 *التوقع:* **${prediction.type}**
                    📈 *الاحتمالية:* ${prediction.probability}%
                    💰 *الرهان المقترح:* ${session.currentBet}$
                    
                    💡 *التحليل:* ${prediction.reasoning}
                    
                    #AI_GOAL #PREDICTION
                `;
                await bot.telegram.sendMessage(CONFIG.CHANNEL_ID, channelMessage, { parse_mode: 'Markdown' });
            } catch (channelError) {
                console.error('Error sending prediction to channel:', channelError);
                // يمكن إرسال إشعار للإدمن بفشل الإرسال
                await bot.telegram.sendMessage(CONFIG.ADMIN_ID, `⚠️ فشل إرسال التوقع إلى القناة ${CONFIG.CHANNEL_ID}. يرجى التحقق من صلاحيات البوت.`);
            }
            ...
            await ctx.replyWithMarkdown(
                `🤖 *النظام الاحتياطي - تحليل فوري (فشل الذكاء الاصطناعي الخارجي)*\n\n` +
...
// 🎯 HANDLE CALLBACK QUERIES (Code remains the same)
...
// 🎯 HANDLER FUNCTIONS (Code remains the same)
...
async function handleNextPrediction(ctx, userData) {
    try {
        ...
        // هنا لا نحتاج لتحليل الصورة مرة أخرى، فقط نستخدم التوقع الداخلي
...
async function handleUserStats(ctx, userData) {
    const accuracy = userData.correct_predictions > 0 ? Math.round((userData.correct_predictions / (userData.total_predictions || 1)) * 100) : 0;
...
async function handleBotStats(ctx) {
    ...
    await ctx.replyWithMarkdown(
        ...
        `🎯 *نظام الذكاء الاصطناعي v${goalAI.algorithmVersion}*`,
        getMainKeyboard()
    );
}
...
async function handleSubscriptions(ctx, userData) {
    ...
    let message = `💳 *باقات الاشتراك*\n\n`;
    
    const typesMap = {
        week: 'أسبوعي (7 أيام)',
        month: 'شهري (30 يوم)',
        three_months: '3 أشهر (90 يوم)',
        year: 'سنوي (365 يوم)'
    };
    ...
    message += `\n\n🔐 *حالتك الحالية:* ${userData.subscription_status === 'active' ? `نشط (${userData.subscription_type})` : `مجاني`}`;
    
    await ctx.replyWithMarkdown(message, getSubscriptionKeyboard());
}
...
async function handleSubscriptionSelection(ctx, userData, text) {
    try {
        ...
        if (type.includes('أسبوعي')) {
            subscriptionType = 'week';
        } else if (type.includes('شهري')) {
            subscriptionType = 'month';
        } else if (type.includes('3') || type.includes('ثلاثة')) {
            subscriptionType = 'three_months';
        } else if (type.includes('سنوي')) {
...
async function handlePaymentScreenshot(ctx, userId) {
    try {
        ...
        // 1. رفع الصورة إلى ImgBB (أو أي خدمة أخرى)
        const uploadResult = await imgbbUploader.uploadImage(imageUrl);
        const uploadedUrl = uploadResult.success ? uploadResult.url : imageUrl; // استخدام الرابط الأصلي كاحتياط

        // 2. تسجيل طلب الدفع
        const paymentData = {
            user_id: userId,
            username: userData.username || ctx.from.first_name,
            onexbet: userData.onexbet || session.paymentAccount,
            subscription_type: session.paymentType,
            amount: prices[session.paymentType],
        ...
        const adminMessage = `
            ...
            📸 *إثبات الدفع:*
            [رابط الصورة](${uploadedUrl})
        `;

        const adminKeyboard = Markup.inlineKeyboard([
            [
                Markup.button.callback('✅ قبول وتفعيل الاشتراك', `accept_${paymentId}`),
                Markup.button.callback('❌ رفض الطلب', `reject_${paymentId}`)
            ]
        ]);
        ...
        await ctx.replyWithMarkdown(
            ...
            `🆔 رقم الطلب: \`${paymentId}\`\n` +
            `⏰ *جاري مراجعة طلبك وتفعيله*` +
            '⏰ عادةً خلال 24 ساعة\n\n' +
            `📞 للاستفسار: ${CONFIG.DEVELOPER}`, getMainKeyboard()
        );
...
// 🔧 ADMIN HANDLERS (All admin handlers remain the same as in your original code)
    ...
    try {
        switch (text) {
            ...
            case '📢 إرسال إشعار':
                ...
                await ctx.replyWithMarkdown('📢 *أرسل الرسالة الآن (سيتم إرسالها لجميع المستخدمين):*', getAdminMainKeyboard());
                break;
            case '🔍 بحث عن مستخدم':
                ctx.session.adminStep = 'search_user';
                await ctx.replyWithMarkdown('🔍 *أرسل معرف المستخدم، أو رقم 1xBet، أو اسم المستخدم (Username)*:', getAdminMainKeyboard());
                break;
            case '🔧 قفل/فتح البوت':
...
async function handleAdminBroadcast(ctx, message) {
    try {
        ...
        ctx.session.adminStep = 'main';
        await ctx.deleteMessage(broadcastMsg.message_id).catch(() => {});
    } catch (error) {
...
async function handleAdminToggleMaintenance(ctx) {
    try {
        const isCurrentlyMaintenance = dbManager.isMaintenanceMode();
        const newState = !isCurrentlyMaintenance;
        await dbManager.setMaintenanceMode(newState);
        
        const status = newState ? 'مقفل' : 'مفتوح';
        await ctx.replyWithMarkdown(`🔧 *حالة البوت:* تم تغييرها إلى **${status}**`, getAdminMainKeyboard());
    } catch (error) {
...
// ... (Other Admin Payment, User, and Settings Handlers are long but remain the same)
// **ملاحظة: لضمان أن الكود يعمل، يجب أن يحتوي ملفك على جميع دوال الـ Admin
// (مثل handleAdminUsers, handleAdminPayments, handlePaymentAccept, handleAdminPriceEdit, إلخ)
// كما هي موجودة في ملفك الأصلي الذي أرسلته.**

async function handlePaymentAccept(ctx, paymentId) {
    try {
        const payment = await dbManager.getPayment(paymentId);
        if (!payment || payment.status !== 'pending') {
            await ctx.answerCbQuery('❌ طلب الدفع غير موجود أو تمت معالجته مسبقاً');
            return;
        }
        ...
        userData.free_attempts = 0; // إلغاء المحاولات المجانية عند الترقية
        
        await dbManager.saveUser(payment.user_id, userData);
        await dbManager.updatePayment(paymentId, { status: 'accepted', processed_at: new Date().toISOString() });
        ...
        try {
            await bot.telegram.sendMessage(
                ...
                `✨ *يمكنك الآن استخدام خدمة التوقعات بدون حدود!*`,
                { parse_mode: 'Markdown' }
            );
        } catch (error) {
        ...
        await ctx.answerCbQuery('✅ تم قبول الطلب وتفعيل الاشتراك');
...
async function handlePaymentReject(ctx, paymentId) {
    try {
        ...
        await dbManager.updatePayment(paymentId, { status: 'rejected', processed_at: new Date().toISOString() });

        // إشعار المستخدم
        try {
            await bot.telegram.sendMessage(
                payment.user_id,
                `❌ *عفواً، تم رفض طلب الدفع الخاص بك*\n\n` +
