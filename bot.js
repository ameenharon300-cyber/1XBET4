...
// 🔥 FEATURES: ULTRA STRICT IMAGE VALIDATION + FULL ADMIN PANEL
// ===================================================
...
// 🎯 ULTRA STRICT IMAGE VALIDATION WITH OPENAI VISION
class UltraStrictImageValidator {
    ...
    async validateImage(imageUrl) {
        try {
            ...
            if (!this.openaiApiKey) {
                console.log('⚠️ لا يوجد مفتاح OpenAI، استخدام التحقق الأساسي');
                return await this.basicValidation(imageUrl);
            }

            // استخدام OpenAI Vision للتحقق الصارم
            const visionResult = await this.validateWithOpenAIVision(imageUrl);
            
            if (!visionResult.valid) {
                return {
                    valid: false,
                    message: '❌ *رفض إجباري - هذه ليست صورة لعبة GOAL* 🚫\n\n' +
                            '📋 *السبب:* ' + visionResult.reason + '\n\n' +
                            '🎮 *المتطلبات الإلزامية:*\n' +
                            '• لاعبين مشهورين (ميسي، رونالدو، نيمار)\n' +
                            '• نص "GOAL" أو "هدف"\n' +
                            '• أزرار "هدف" و "لا هدف"\n' +
                            '• زر "وضع الرهان"\n' +
    ...
    async validateWithOpenAIVision(imageUrl) {
        try {
            console.log('👁️ جاري التحقق باستخدام الذكاء الاصطناعي...');
...
المتطلبات الإلزامية (يجب توفرها جميعاً):
1. لاعبين كرة قدم مشهورين (ميسي، رونالدو، نيمار) - يجب أن يكونا واضحين
2. نص كبير "GOAL!" أو "هدف" في الأعلى
3. أزرار خضراء وحمراء مكتوب عليها "هدف" و "لا هدف"
4. زر "وضع الرهان"
5. واجهة لعبة كرة القدم وليست مباراة حقيقية
...
أجب بدقة بـ "مقبولة" فقط إذا كانت كل الشروط متوفرة بوضوح، أو "مرفوضة" إذا كان هناك أي نقص.`
                            },
                            {
                                type: "image_url",
                                image_url: imageUrl
            ...
            if (answer.includes('مقبولة') || answer.includes('نعم') || answer.includes('صحيح') || answer.includes('yes')) {
                return {
                    valid: true,
                    confidence: 0.98,
                    reason: 'تم التحقق من جميع المتطلبات الإلزامية',
                    details: {
                        players: true,
                        goal_text: true,
                        buttons: true,
                ...
                return {
                    valid: false,
                    confidence: 0.95,
                    reason: 'الصورة لا تحتوي على جميع متطلبات لعبة GOAL',
                    details: {
                        players: false,
                        goal_text: false,
                        buttons: false,
    ...
    async basicValidation(imageUrl) {
        try {
            console.log('📝 استخدام التحقق الأساسي...');
            ...
            const requiredKeywords = ["goal", "هدف", "لا هدف", "وضع الرهان"];
            const foundKeywords = requiredKeywords.filter(word => cleanText.includes(word.toLowerCase()));

            console.log(`🔍 الكلمات الموجودة: ${foundKeywords.length} من ${requiredKeywords.length}`);

            if (foundKeywords.length >= 3) {
                return {
                    valid: true,
                    confidence: 0.85,
                    reason: `تم العثور على ${foundKeywords.length} كلمة مفتاحية`,
                ...
                return {
                    valid: false,
                    confidence: 0.8,
                    reason: `لم يتم العثور على الكلمات المطلوبة (${foundKeywords.length} من 3)`,
                    details: { foundKeywords }
                };
            }
        } catch (error) {
            ...
            return {
                valid: false,
                confidence: 0.5,
                reason: 'فشل في التحقق من الصورة'
            };
        }
    }
