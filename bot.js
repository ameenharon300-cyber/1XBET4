...
[file content begin]
// ===================================================
// 🚀 AI GOAL PREDICTOR ULTIMATE - VERSION 9.1
// 👤 DEVELOPER: AMIN - @GEMZGOOLBOT
// 🔥 FEATURES: SMART AI + BETTING SYSTEM + FIREBASE + FULL ADMIN PANEL
// ===================================================

console.log('🤖 Starting AI GOAL Predictor Ultimate v9.1...');
...
const CONFIG = {
    ...
    DEVELOPER: "AMIN - @GEMZGOOLBOT",
    CHANNEL: "@GEMZGOOL",
    START_IMAGE: "https://i.imgur.com/7WIoYXh.jpg",
...
🎯 *استخدم أزرار الفوز/الخسارة في الأسفل للإبلاغ عن النتيجة*
            `;
            ...
            const resultKeyboard = Markup.inlineKeyboard([
                [
                    Markup.button.callback(`🎉 فزت - ربح ${session.currentBet * 2}$`, `win_${userData.total_predictions}`),
                    Markup.button.callback(`💔 خسرت - ضعف الرهان ${session.currentBet * 2}$`, `lose_${userData.total_predictions}`)
                ]
            ]);

            await ctx.replyWithMarkdown(
                '📊 *ما هي نتيجة التوقع؟*\n\n' +
                `🎉 *فزت* - تربح ${session.currentBet * 2}$ (رأس المال + الربح)\n` +
                `💔 *خسرت* - تخسر ${session.currentBet}$ وتضاعف الرهان لـ ${session.currentBet * 2}$\n\n` +
    ...
    try {
        ...
        if (callbackData.startsWith('win_') || callbackData.startsWith('lose_')) {
            ...
            if (isWin) {
                ...
                await ctx.answerCbQuery(`🎉 تم تسجيل الفوز! ربحت ${profit}$`);
                ...
                await ctx.replyWithMarkdown(
                    ...
                    `🎯 يمكنك الآن البدء بتوقع جديد من خلال زر "التوقع التالي"`,
                    getMainKeyboard()
                );
                ...
                // الخسارة - مضاعفة الرهان
                const newBet = ctx.session.currentBet * 2;
                userData.losses = (userData.losses || 0) + 1;
                ctx.session.currentBet = newBet;
                
                await ctx.answerCbQuery(`💔 تم تسجيل الخسارة. الرهان الجديد: ${newBet}$`);
                
                // إرسال رسالة الخسارة
                await ctx.replyWithMarkdown(
                    `💔 *للأسف خسرت*\n\n` +
                    `❌ توقعك كان خاطئاً\n` +
                    `💰 خسرت: ${ctx.session.currentBet / 2}$\n` +
...
// ... باقي دوال الإدمن تبقى كما هي بدون تغيير (للتقصير في الطول)

// 🚀 START BOT
bot.launch().then(() => {
    console.log('🎉 SUCCESS! AI GOAL Predictor v9.1 is RUNNING!');
