// ===================================================
// 🚀 AI GOAL PREDICTOR ULTIMATE - VERSION 10.3
// 👤 DEVELOPER: AMIN - @GEMZGOOLBOT
// 🔥 FEATURES: SMART AI + BETTING SYSTEM + FIREBASE + FULL ADMIN PANEL
// ===================================================

console.log('🤖 Starting AI GOAL Predictor Ultimate v10.3...');
...
const CONFIG = {
    ...
    ALLOWED_IMAGE: "https://i.ibb.co/VYjf05S0/Screenshot.png", // الصورة المسموحة فقط
...
// 🔥 FIREBASE INITIALIZATION - الإصدار المصحح
let db = null;
let admin = null;
...
try {
    ...
    // استخدام التهيئة الصحيحة لـ Firebase Admin SDK
...
// 🗄️ LOCAL STORAGE FALLBACK - مبسط وموثوق
...
class ImgBBUploader {
    ...
    async uploadImage(imageUrl) {
        try {
            ...
            return {
                success: true,
                url: imageUrl, // استخدام الرابط الأصلي
                delete_url: imageUrl
            };
        } catch (error) {
            return {
...
bot.use(session({ 
    defaultSession: () => ({ 
        ...
        lastImageUrl: null // تخزين آخر صورة تم تحليلها
    })
}));
...
// 🖼️ IMAGE ANALYSIS HANDLER - معدل للتحقق من الصورة المسموحة فقط
bot.on('photo', async (ctx) => {
    try {
        ...
        // 🔍 التحقق من أن الصورة هي المسموحة فقط
        if (!imageUrl.includes('Screenshot.png') && !imageUrl.includes('i.ibb.co/VYjf05S0')) {
            await ctx.replyWithMarkdown(
                '❌ *صورة غير مسموحة!*\n\n' +
                '📸 *يجب استخدام صورة المباراة الرسمية فقط*\n\n' +
                '🖼️ الصورة المسموحة: ' + CONFIG.ALLOWED_IMAGE,
                getMainKeyboard()
            );
            return;
        }
...
📝 *الأوامر المتاحة:*
• /toggle_maintenance - قفل/فتح البوت
        `;
...
// 🚀 START BOT
bot.launch().then(() => {
    console.log('🎉 SUCCESS! AI GOAL Predictor v10.3 is RUNNING!');
